...
GenericActionCommand::GenericActionCommand(
    ...
    setRequestId(RequestIdentifierPtr(new RequestIdentifier(requestId)));
...
bool GenericActionCommand::equals(AgObject *obj)
{
    GenericActionCommand* gac = dynamic_cast<GenericActionCommand*>(obj);
    if (gac == 0) {
        return false;
    }
    ...
    } else if(!requestId->equals(gac->requestId.get())) {
        return false;
    }
